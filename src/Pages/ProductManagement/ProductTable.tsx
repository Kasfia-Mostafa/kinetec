...
import { useGetProductsQuery, useDeleteProductMutation, useUpdateProductMutation } from "@/redux/api/baseApi";
...
const ProductTable = () => {
  ...
  const [updateProduct, { isLoading: isUpdating, isError: updateError, isSuccess: updateSuccess, error: updateErrorDetail }] = useUpdateProductMutation();
    ...
    <div>
      <div className="ml-20">
        <Link to={`/newProduct`}>
          ...
          transform rounded-lg border-2 border-yellow-500 text-md
          text-yellow-500 before:absolute before:top-0 
          before:block before:h-0 before:w-full before:duration-500
          hover:text-white hover:before:absolute hover:before:left-0 
          hover:before:-z-10 hover:before:h-full
          hover:before:bg-yellow-500">Add new product</button>
        </Link>
      </div>
      ...
      {editProduct && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-75">
          <div className="bg-white p-6 rounded-lg shadow-md w-[600px]">
            <h2 className="text-xl font-bold mb-4 text-yellow-600">Edit Product</h2>
            <form onSubmit={handleUpdateSubmit}>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-500">Image URL</label>
                <input
                  type="text"
                  className="mt-1 p-2 h-8 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-500">Price</label>
                <input
                  type="number"
                  step="0.01"
                  className="mt-1 p-2 h-8 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-500">Quantity</label>
                <input
                  type="number"
                  className="mt-1 p-2 h-8 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-500">Rating</label>
                <input
                  type="number"
                  step="0.1"
                  min="0"
              ...
              {updateError && <p className="text-red-500 mt-2">Failed to update the product: {updateErrorDetail?.message || 'Unknown error'}</p>}
              {updateSuccess && <p className="text-green-500 mt-2">Product updated successfully!</p>}
            </form>
          </div>
        </div>
      )}
